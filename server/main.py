...
1. Serves the configuration UI
2. Manages provider accounts
3. Generates LiteLLM config.yaml
4. Manages LiteLLM proxy as a subprocess
"""
...
def start_litellm_proxy(port: int = 11434) -> bool:
    ...
    env = os.environ.copy()
    env.update(get_litellm_env_vars())
    ...
    try:
        ...
        if _litellm_process.poll() is not None:
            # Process died
            output = _litellm_process.stdout.read() if _litellm_process.stdout else ""
...
async def lifespan(app: FastAPI):
    ...
    print(f"[INFO] LiteLLM proxy will run on: http://localhost:{port}", flush=True)
...
async def get_provider_accounts():
    """List all saved accounts (without exposing API keys)."""
    accounts = get_accounts()
    safe_accounts = [
        {
            "accountName": acc.get("accountName", ""),
